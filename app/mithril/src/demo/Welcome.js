...
const Main = {
  view: vnode =>
    ...
    }}>{vnode.children}</article>,
};

const Title = {
  view: vnode =>
    <h1>{vnode.children}</h1>
};
...
const StoryLink = {
  ...
  view: vnode =>
    <a href={vnode.state.href} style={{
      color: '#1474f3',
      textDecoration: 'none',
      borderBottom: '1px solid #1474f3',
      paddingBottom: '2px',
    }} onclick={vnode.state.onclick}>{vnode.children}</a>,
};
  ...
  view: vnode =>
    <Main>
      ...
      <p>
        ...
        {' '}
        {vnode.attrs.showApp
          ? <Link onclick={vnode.attrs.showApp} role="button" tabIndex="0">stories</Link>
          : <StoryLink kind={vnode.attrs.showKind} story={vnode.attrs.showStory}>stories</StoryLink>
        }
        {' '}
